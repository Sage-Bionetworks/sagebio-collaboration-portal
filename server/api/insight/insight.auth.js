...
export function canCreateInsight() {
    return auth.hasEntityPermission(attachInsightAuthorizationDetailsForCreate(), [
...
export function canDeleteInsight() {
    return auth.hasEntityPermission(attachInsightAuthorizationDetails(), [accessTypes.ADMIN.value]);
}
...
function attachInsightAuthorizationDetails() {
    return compose().use((req, res, next) => Insight.findById(req.params.id, 'projectId createdBy')
        .exec()
        .then(insight => {
            if (!insight) {

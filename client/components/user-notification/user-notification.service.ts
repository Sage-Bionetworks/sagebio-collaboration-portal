...
export class UserNotificationService {
    ...
    query(query?: {}): Observable<UserNotification[]> {
        return this.httpClient.get<UserNotification[]>(`/api/user-notifications${stringifyQuery(query)}`);
    }

    create<N extends UserNotification>(notification: N): Observable<N> {
        return this.httpClient.post<N>('/api/user-notifications/', notification);
    }

    archive(notification: UserNotification): Observable<UserNotification> {
